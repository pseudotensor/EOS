...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  double lrho_max = 9.0;
  double lrho_stp = (lrho_max-lrho_min)/(double(Nstps)-1);

  double lT_min = 2.0;
  double lT_max = 7.0;
  double lT_stp = (lT_max - lT_min)/(double(Nstps)-1);
  ...
  for (size_t j=0; j<Nstps; ++j)
    for (size_t i=0; i<Nstps; ++i){
      double rho = std::pow(10.0, lrho_min + j*lrho_stp);
      double Te = std::pow(10.0, lT_min + i*lT_stp);
	...
	fb = cef(muemax);
      ...
      std::cerr << "Done with bracketing! "
		...
		<< std::setw(15) << fb << " @" << std::setw(15) << muemax*T
		<< "\n";
      */
