...
c
c
c BELOW need never be touched by user (only change from original eosfxt.f is h->hcgs for the parameter in const.dek)
c
c as with jon_helm,  made several things single globals
c As related to above, Made storage a subroutine
c       call store_row(j)
c
c
c
...
c..totals
c JCM:
c      double precision pres,ener,entr,
c     1                 dpresdd,dpresdt,dpresda,dpresdz,
c     2                 denerdd,denerdt,denerda,denerdz,
c     3                 dentrdd,dentrdt,dentrda,dentrdz


c..radiation
      integer          radmult
c JCM:
c      double precision prad,erad,srad,
c     1                 dpraddd,dpraddt,dpradda,dpraddz,
c     2                 deraddd,deraddt,deradda,deraddz,
c     3                 dsraddd,dsraddt,dsradda,dsraddz
...
c JCM:
      double precision
c pion,eion,sion,xni,
     1                 etaion,
c     1                 dpiondd,dpiondt,dpionda,dpiondz,
c     2                 deiondd,deiondt,deionda,deiondz,
c     3                 dsiondd,dsiondt,dsionda,dsiondz,
c     4                 dxnidd,dxnidt,dxnida,dxnidz,
     5                 detaiondd,detaiondt,detaionda,detaiondz 


c..electron-positrons
c JCM: (now these things are single globals in vector_eos.single.dek)
      integer          elemult
c      double precision
...
c     5                 pele,
c     5                 dpeledd,dpeledt,dpeleda,dpeledz,
c     6                 ppos,
c     6                 dpposdd,dpposdt,dpposda,dpposdz,
c     7                 pep,
c,dpepdd,dpepdt,dpepda,dpepdz,
c     8                 eele,
c     8                 deeledd,deeledt,deeleda,deeledz,
c     9                 epos,
c     9                 deposdd,deposdt,deposda,deposdz,
c     1                 eep,
c,deepdd,deepdt,deepda,deepdz,
c     2                 sele,
c     2                 dseledd,dseledt,dseleda,dseledz,
c     3                 spos,
c     3                 dsposdd,dsposdt,dsposda,dsposdz,
c     4                 sep
...
c      double precision eip,deipdd,deipdt,deipda,deipdz,
c     1                 sip,dsipdd,dsipdt,dsipda,dsipdz


c..coulomb corrections
      integer          coulmult
c      double precision plasg,
c     1                 pcoul,dpcouldd,dpcouldt,dpcoulda,dpcouldz,
c     2                 ecoul,decouldd,decouldt,decoulda,decouldz,
c     3                 scoul,dscouldd,dscouldt,dscoulda,dscouldz



c..various physical quantities based on derivatives 
c      double precision chit,chid,cv,cp,gam1,gam2,gam3,nabad,sound


c..for the maxwell relations
c      double precision dse,dpe,dsp


c..miscelaneous local variables 
      integer          i,j,niter,mode
      double precision kt,ktinv,x,y,z,
c xx,
     1                 yy,
c zz,
     1                 ages,agesav,agesnew,
...
c       write(*,*) 'EOSFXT Got:',temp,den,abar,zbar
...
c JCM: added below xnem assignment as in HELM so consistent with HELM
c..assume complete ionization 
c       xnem    = xni * zbar
       xnem = xne
c JCM: Made storage a subroutine
       whichnonnucleareos=2
       storenewspecies=1
...
c..both timmes eos routine eosfxt and eosfxt2
c..use this for their thermodynamics of the electrons and positrons
...
c JCM:
c      double precision xconst,pconst,econst,mecc,dbetadt,safe
      double precision xconst,pconst,econst,dbetadt,safe
      parameter        (xconst  = 2.4883740912221807d30,
     1                  pconst  = 1.3581730208282635d24,
     2                  econst  = 2.0372595312423953d24, 
c     3                  mecc    = me * clight * clight,
     4                  dbetadt = kerg/mecc,
     5                  safe    = 0.005d0)
...
c..both timmes eos routine eosfxt and eosfxt2
c..use this for their coulomb correction
...
c..for the uniform background coulomb correction
     ...
     4                 aion,
     5                 plasgdd,plasgdt,plasgda,plasgdz,
     6                 x,y,z
...
c..the various timmes eos routines, eosfxt, eosfxt2, eosion
c..all use this to make an initial guess at the chemical potential
...
c..the various timmes eos routines, eosfxt, eosfxt2, eosion
c..all use these fermi-dirac functions
...
c JCM: Went to 18 figures, no change
