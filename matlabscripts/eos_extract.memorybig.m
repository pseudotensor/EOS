...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % number of header entries without whichrnpmethod
  numheaders=16;
  % +1 gets whichrnpmethod again
  %[myhead,count]=fscanf(fid,'%g',[numheaders+1]);

  
  % I don't understand why this way can't be done, gets stuck at ntk
  fid=fopen(file1,'rt');
  [myhead,count]=fscanf(fid,'%d',[6]);
  ii=1;
  ...
  %[mydata,count]=fscanf(fid,'%g',[nc*nrhob*ntk]);
  fclose(fid);
  ...
  if smoothinputs
    ...
    for q=1:nhcm
      for r=1:ntdynorye
        ...
        ptot(:,:,q,r) = moving_average2(roughptot(:,:),1,1);
        utot(:,:,q,r) = moving_average2(roughutot(:,:),1,1);
        stot(:,:,q,r) = moving_average2(roughstot(:,:),1,1);
  ...
  fprintf('Outputting monotinized EOS');
  ...
  lutotdiffoutmin=lutotdiffmin;
  lutotdiffoutmax=lutotdiffmax;
  ...
  lptotdiffoutmin=lptotdiffmin;
  lptotdiffoutmax=lptotdiffmax;
  ...
  lchidiffoutmin=lchidiffmin;
  lchidiffoutmax=lchidiffmax;
  ...
  clear extraofUdiff;
  extraofUdiff=zeros(nrhob,nutotdiff,nhcm,ntdynorye,numextras);
  ...
  NUMOUTCOLUMNS=4+1+3+2+2+2+1+3+3+3+numextras; % 24 + numextras
      ...
      for n=1:nutotdiffout
        for m=1:nrhob
          fprintf(fid,'%3d %3d %3d %3d %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g', ...
                  m-1, n-1, o-1, p-1, ...
                  rhob(m,n,o,p), utotdiffoutgrid(n), ptotdiffoutgrid(n), chidiffoutgrid(n), hcm(m,n,o,p), tdynorye(m,n,o,p), ...
          ...
          % mynewdata(:,:,:,:,ei)
          for ei=1:numextras
            fprintf(fid,'%21.15g ',extraofUdiffout(m,n,o,p,ei));
          end
          fprintf(fid,'\n');
        end
      end
  ...
  fprintf(fid,'%d %d %d\n',whichrnpmethod,NUMOUTCOLUMNS,numextras);
