...
c      Program reaction_rate_p_electron  !check2
      real*8 function gammane(T11,etae)
...
c      read(*,*) T11,etae        !check2
      ...
      dum(1)=xintd(0.d0,1.d0,integgammane,iter)
      dum(2)=xintd(1.d0,10.d0,integgammane,iter)
      dum(3)=xintd(10.d0,100.d0,integgammane,iter)
      dum(4)=xintd(100.d0,1000.d0,integgammane,iter)
      dum(4)=xintd(1000.d0,10000.d0,integgammane,iter)
      dum(5) = dum(1) + dum(2) + dum(3) + dum(4)


      dum(10)=GF**2/(2.d0*PI**3)*(1.d0+3.d0*ga**2)*tmev**5 !in MeV
      dum(11) =  dum(10) * mevtosec ! in 1/sec
      gammane= dum(11) * dum(5)
...
C======================================================================
C----------------------------------------------------------------------
      tmp(1)= Sqrt(x*(x+2.d0*metilde)) * (x+metilde)*(x+metilde+Qtilde)**2
      tmp(2)=exp(x+Qtilde + etae1) + 1.d0
      tmp(3)=1.d0/ (1.d0 + exp(- (x+metilde +Qtilde)))
      integgammane=tmp(1) / tmp(2)
